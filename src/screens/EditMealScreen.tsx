...
export default function EditMealScreen() {
    ...
    useEffect(() => {
        if (mealToEdit) {
            ...
            Alert.alert('Error', 'No se encontró comida para editar.');
            navigation.goBack();
        }
    }, [mealToEdit]);
    ...
    const handleAddIngredient = () => {
        ...
        if (!grams || grams <= 0) return Alert.alert('Error', 'Ingresa una cantidad válida.');
        const _id = getObjectIdFromMongoDoc(selectedFood._id);
        if (ingredients.some(i => i.food_id === _id))
            return Alert.alert('Error', 'Este alimento ya está en la lista.');
    ...
    const handleUpdateMeal = async () => {
        if (!user) {
            return Alert.alert('Error', 'No se pudo obtener la información del usuario.');
        }
        if (!mealName.trim()) return Alert.alert('Error', 'Debes nombrar la comida.');
            ...
            >
                ...
                >
                    ...
                    <View style={styles.header}>
                        <Text style={styles.headerSubtitle}>Editar</Text>
                        <Text style={styles.headerTitle}>Modificar Comida</Text>
                        <Text style={styles.headerDescription}>
                            Actualiza los ingredientes y preferencias de tu comida.
                        </Text>
                    </View>
...
const createDynamicStyles = (colors: any, darkMode: boolean) => StyleSheet.create({
    ...
    saveButton: { backgroundColor: colors.primary, borderRadius: 28, paddingVertical: 18, alignItems: 'center', shadowColor: colors.primary, shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.2, shadowRadius: 8, elevation: 4 },
    saveButtonDisabled: { opacity: 0.7 },
    buttonContent: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center' },
    saveButtonText: { fontSize: 17, fontWeight: '700', color: colors.onPrimary, marginRight: 8 },
});